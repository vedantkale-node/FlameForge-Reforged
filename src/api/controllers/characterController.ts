...
const MINIMAL_PROJECTION = '-_id -__v -talents -constellations -stories -voiceLines -cdata -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -affiliation -constellation';
...
const getSingleCharacter = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const projectionObj: Record<string, number> = isFull
            ? { __v: 0, _id: 0, cdata: 0 }
            : {
                __v: 0, _id: 0, talents: 0, constellations: 0, stories: 0, voiceLines: 0, cdata: 0,
